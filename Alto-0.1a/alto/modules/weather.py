...
def handle(text, state=None):
    ...
    Accepts state (for compatibility) but ignores it.
    ...
    try:
        ...
        if not location:
            return "I couldn't determine your location. Please try again later.", state
        ...
        if not weather:
            return f"I found your location ({city}, {country}) but couldn't fetch the weather data.", state
        ...
        return "Sorry, I encountered an error while fetching the weather.", state
