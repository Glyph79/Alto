...
def handle(text: str, state: dict) -> (str, dict):
    """
    Weather module with IP‑based location and Open‑Meteo.
    Returns (response, updated_state). Does not use multi‑turn in this example,
    so state is returned unchanged (empty dict).
    """
    try:
        ...
        response = format_weather_response(city, country, weather)
        return response, {}
...
def get_location_from_ip():
    """Get approximate location from user's IP address using ip-api.com."""
    try:
...
def get_weather(lat, lon):
    """Get current weather for coordinates using Open‑Meteo."""
    try:
...
def format_weather_response(city, country, weather):
    ...
    if temp is not None:
        response += f"Temperature is {temp}°C. "
